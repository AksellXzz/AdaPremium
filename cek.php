...
<head>
    ...
    <title>Lacak Pesanan — <?= defined('STORE_NAME') ? STORE_NAME : 'Ada Premium' ?></title>
...
<nav class="sticky top-0 z-50 glass py-4 px-6 md:px-10 flex justify-between items-center border-b border-white/5 shadow-sm w-full">
    <a href="/" class="text-2xl font-extrabold tracking-tighter text-white"><?= defined('STORE_NAME') ? explode(' ', STORE_NAME)[0] : 'PREMIUM' ?><span class="text-violet-500">.</span></a>
    ...
    <div class="w-full max-w-lg">
        ...
        ?>
            ...
            <div class="glass rounded-[2.5rem] p-6 md:p-10 shadow-2xl bg-[#111827]/50 animate-up">
                ...
                <?php if($r['status'] != 'pending'): ?>
                    <a href="https://wa.me/<?= defined('WA_ADMIN') ? WA_ADMIN : '6285729889079' ?>?text=Halo+Admin+Bantu+Cek+Inv+<?= htmlspecialchars($r['invoice']) ?>" target="_blank" class="mt-6 block text-center bg-slate-800 hover:bg-slate-700 py-4 rounded-2xl font-bold text-xs tracking-widest text-white uppercase transition">Ada Kendala? Hubungi Admin</a>
                <?php endif; ?>
...
<a href="https://wa.me/<?= defined('WA_ADMIN') ? WA_ADMIN : '6285729889079' ?>" target="_blank" class="fixed bottom-6 right-6 z-50 bg-green-500 text-white w-14 h-14 rounded-full flex items-center justify-center text-3xl shadow-lg shadow-green-500/40 hover:scale-110 hover:-translate-y-1 transition-all">
